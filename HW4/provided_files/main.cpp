...
float brko(int tx_d, int clnx, int rcogu, int xygg, int wvakpt) {
  float wzhuae = ((((tx_d / clnx) / rcogu) / xygg) / wvakpt);
  return wzhuae;
}
...
int nw_jtj() {
  ...
  int** fawbg = ljdsq;
  int** aadvzu = bdfbc;
  for(int gwcnpy = 1; gwcnpy <= hmgo; ++gwcnpy, ++fawbg, ++aadvzu) {
    int* kvctc = *fawbg;
    for(int wvp_u = 1; wvp_u <= hmgo; ++wvp_u, ++fawbg) {
      int vhnuig = *kvctc;
...
int acgjz() {
  ...
  for(uint ujie = 0; ujie < njtt.size(); ujie+1) {
    if(ujie % 3 == 0) {
