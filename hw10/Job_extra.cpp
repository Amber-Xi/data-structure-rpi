...
//Removes a job from whichever ElegantQueue it belongs to by the urgent
void Job::removeFromUrgent() { 
    ptr->urgent_remove_from_index(urgent_hook); 
}

//Removes the job from whichever ElegantQueue it belongs to by the time
void Job::removeFromTime() { 
    ptr->time_remove_from_index(time_hook); 
}
