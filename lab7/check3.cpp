...
int count_path(vector<vector<bool> > const& blocked_grid, 
  int start_x, int start_y, int step, vector <int> &path_record)
{
  ...
  if (start_x < 0 || start_x > blocked_grid.size() || start_y < 0
  || start_y > blocked_grid[0].size()) 
  {
    return 0;
  }
  if (blocked_grid[start_x][start_y]) 
  {
    return 0;
  }
  path_record.push_back(start_x);
  path_record.push_back(start_y);
  //std::cout << "(" <<start_x << "," << start_y << ")" << std::endl;

  if (start_x == 0 && start_y == 0)
  { 
    std::cout << "HOORAY! " << "Steps Taken: " << step << std::endl;
    return 1;
  }
  step++;
  return count_path(blocked_grid, start_x-1, start_y, step, path_record)
  + count_path(blocked_grid, start_x, start_y-1, step, path_record);

}

int count_path(vector<vector<bool> > const& blocked_grid, 
  int start_x, int start_y, vector <int> &path_record)
  {
    return count_path(blocked_grid, start_x, start_y, 0, path_record);
  }
...
int main(int argc, char* argv[]) {
  ...
  // Read the new starting point.
  std::cout << "Please input the starting point (x, y): " << std::endl;
  std::cin >> start_x >> start_y;
  if (blocked_grid[start_x][start_y] == true)
  {
    std::cerr << "WARNING: Starting point was set on a blocked location."
    << std::endl;
    exit(1);
  }
  ...
  int possiblity = count_path(blocked_grid, start_x, start_y, path_record);
  
  if (possiblity == 0)
    path_record.clear();
  else
  {
    for (int i = path_record.size()-2; i >= 0; i-=2)
    {
      if (path_record[i] == 0 && path_record[i+1] == 0)
        break;
      path_record.pop_back(); path_record.pop_back();
    }
  }

  std::cout << "Number of possible paths: " << possiblity << std::endl;
  if (possiblity != 0)
  {
  int count = 1;
  std::cout <<  "Route " << count << ": " << std::endl;
  
  for (int i = 0; i + 2 <= path_record.size(); i+=2)
  {
    std::cout << "(" <<path_record[i] << "," 
    << path_record[i+1] << ")" << std::endl;
    if (path_record[i] == 0 && path_record[i+1] == 0 && count < possiblity)
    {
      count++; std::cout <<  "Route " << count << ": " << std::endl;
    }
  }
  }
