...
int countPaths(int x, int y, vector<vector<bool> >& blocked_grid, vector<int>& ptx, vector<int>& pty)
{
    int counter = 0;
    if (x<0||y<0)
        return 0;
    ...
    {
        counter += countPaths(x-1, y, blocked_grid, ptx, pty) + countPaths(x, y-1,blocked_grid, ptx, pty);
        return counter;
    }
    
}
void VecPath(int x, int y, vector<vector<bool> > blocked_grid, vector<int>& ptx, vector<int>& pty)
{
    if (countPaths(x, y,blocked_grid, ptx, pty)==0)
        return;
    else
    {
        ptx.push_back(x);
        pty.push_back(y);
        if (countPaths(x,y-1,blocked_grid, ptx, pty)>0)
            VecPath(x, y-1, blocked_grid, ptx, pty);
        else
            VecPath(x-1, y, blocked_grid, ptx, pty);
    }
}
void PrintPath(int x, int y, vector<vector<bool> > blocked_grid, vector<int>& ptx, vector<int>& pty)
{
    if (countPaths(x, y,blocked_grid, ptx, pty)==0)
        return;
    else
    {
        cout << "(" << x << ", " << y << ")" << endl;
        if (countPaths(x,y-1,blocked_grid, ptx, pty)>0)
            PrintPath(x, y-1, blocked_grid, ptx, pty);
        else
            PrintPath(x-1, y, blocked_grid, ptx, pty);
    }
}
...
void print_grid(vector<vector<bool> > const& blocked_grid, int start_x, int start_y, vector<int>& ptx, vector<int>& pty, vector<vector<bool> > const& true_spot) {
...
/*
 void print_grid2(vector<vector<bool> > const& blocked_grid, int start_x, int start_y, vector<int>& ptx, vector<int>& pty) {
 for (int y=0; y<blocked_grid[0].size(); ++y)
 {
 for (int x=0; x<blocked_grid.size(); ++x)
 {
 ...
 cout << " X";
 else if (find(ptx.begin(), ptx.end(), x) != ptx.end())
 {
 bool found=false;
 for (int j=0; j<ptx.size()-1; j+=2)
 {
 if (ptx[j]==x && ptx[j+1]==y)
 {
 found=true;
 break;
 }
...
int main(int argc, char* argv[]) {
    ...
    vector<int> pointsx;
    vector<int> pointsy;
    int count=countPaths(start_x, start_y, blocked_grid, pointsx, pointsy);
    VecPath(start_x, start_y, blocked_grid, pointsx, pointsy);
    
    
    vector<vector<bool> > true_spot;
    for(int i = 0; i < blocked_grid.size(); i++){
        vector<bool> v(blocked_grid[i].size(), false);
        true_spot.push_back(v);
    }
    
    for(int i = 0; i < pointsx.size(); i++){
        true_spot[pointsx[i]][pointsy[i]] = true;
    }
    print_grid(blocked_grid, start_x, start_y, pointsx, pointsy, true_spot);
    //print_grid2(blocked_grid, start_x, start_y, pointsx, pointsy);
    cout << "Number of paths from start to end: " << count << endl;
    PrintPath(start_x, start_y, blocked_grid, pointsx, pointsy);
