...
*/
void print_grid(vector<vector<bool> > const& blocked_grid, int start_x, int start_y, vector<vector<int> > all_out1) {


  for (int y=0; y<blocked_grid[0].size(); ++y) {
    for (int x=0; x<blocked_grid.size(); ++x) {
      for(int i =0; i< all_out1.size(); i++)
      {
        for(int j = 0; j< all_out1[i].size(); j++)
        {
        if(all_out1[i][j])
          cout << '$';
        if (x == start_x && y == start_y)
...
/*
vector<int> out1;
vector<vector<int> > all_out1;
int count_number(const Point& p, vector<vector<bool> > const& blocked_grid) {
    //cout << "1";
    int out = 0;
    Point point = p;
    out1.push_back(point.x);
    out1.push_back(point.y);
    cout << "point: " << point.x << "," << point.y <<endl;
    all_out1.push_back(out1);
    ...
    if(point.x == 0 && point.y > 0) {
        ...
        point.y -= 1;
        //cout << "point: " << point.y << "," << point.x <<endl;
        out += count_number(point, blocked_grid);
    }
    ...
    if(point.x - 1 >= 0 && point.y - 1 >= 0) {
           // make sure that the upper condition does not affect the lower one
        if(check_blocked(Point(p.x-1, p.y), blocked_grid) == true) {
            point.x -= 1;
            //cout << "point: " << point.y << "," << point.x <<endl;
            out += count_number(point, blocked_grid);
        }
        ...
        Point point2 = p;
        if(check_blocked(Point(p.x, p.y-1), blocked_grid) == true) {
            point2.y -= 1;
             //cout << "point: " << point.y << "," << point.x <<endl;

            out += count_number(point2, blocked_grid);
        }
...
int count_number(const Point& p, vector<vector<bool> > const& blocked_grid)
{
  ...
  {
    point.x-=1;
    if(!(check_blocked(point,blocked_grid)))
    {
      out += count_number(point,blocked_grid);
    }
  }
...
// mark if it was blocked
    Point p = Point(start_x,start_y);
    //cout << "point: " << p.y << "," << p.x <<endl;
    int num = count_number(p, blocked_grid);
